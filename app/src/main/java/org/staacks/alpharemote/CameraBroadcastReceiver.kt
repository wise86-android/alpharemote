...
class CameraBroadcastReceiver : BroadcastReceiver() {
    ...
    private fun isExternalBroadcastIntentEnabled(context: Context) = runBlocking {
        SettingsStore(context).getEnableExternalBroadcastControl()
    }
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (!isExternalBroadcastIntentEnabled(context)) {
        ...
        try {
            ...
            val serviceIntent = Intent(context, AlphaRemoteService::class.java).apply {
                ...
                putExtra(
                    AlphaRemoteService.BUTTON_INTENT_CAMERA_ACTION_EXTRA,
                    cameraAction as Serializable
                )
