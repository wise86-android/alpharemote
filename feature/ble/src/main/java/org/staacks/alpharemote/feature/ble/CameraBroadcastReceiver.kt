package org.staacks.alpharemote.feature.ble
...
import org.staacks.alpharemote.feature.ble.data.BehaviorSettings
...
class CameraBroadcastReceiver : BroadcastReceiver() {
        ...
        CoroutineScope(Dispatchers.Default).launch {
            try {
                if (!BehaviorSettings(context).getBroadcastControl()) {
                    Log.w(TAG, "Broadcast control not allowed by user.")
                    return@launch
                }
    ...
    companion object{
        private const val TAG = "CameraBroadcastReceiver"
