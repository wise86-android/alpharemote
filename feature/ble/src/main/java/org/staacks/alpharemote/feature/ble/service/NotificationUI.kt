...
class NotificationUI(private val context: Context) {
    ...
    fun start(): Notification {
        scope.launch {
            appearanceSettings.customButtonSettings.collectLatest {
    ...
    private fun setupNotificationBuilder(): NotificationCompat.Builder {
        // Can't reference MainActivity directly - it lives in the app module, which this one
        // must not depend on. The launcher intent for our own package always resolves to it.
        val intent = context.packageManager.getLaunchIntentForPackage(context.packageName)?.apply {
            addFlags(Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_SINGLE_TOP)
        }
        val pendingIntent = intent?.let {
            PendingIntent.getActivity(context, 42, it, PendingIntent.FLAG_IMMUTABLE or PendingIntent.FLAG_UPDATE_CURRENT)
        }
